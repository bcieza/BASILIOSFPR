...
//	int row = ncross[p1] + 5; //+5 is buffer
	...
	for (c = 0; c < csize; c++) {
		...
		for (i = 0; i < ncross[p1]; i++) {
			p2 = cross_part[p1][i];
			if(ncross[p1]>1){
				if(cross_part[p1][i]==cross_part[p1][i-1] && cross_part[p1][i-1]<4000){
					cout<<"The protein in case "<<p1<<endl;
					cout<<"partner1 "<<cross_part[p1][i]<<endl;
					cout<<"partner2 "<<cross_part[p1][i+1]<<endl;
					cout<<"partner1rxn "<<cross_rxn[p1][i]<<endl;
					cout<<"partner2rxn "<<cross_rxn[p1][i+1]<<endl;
					cout<<"partner1interface "<<cross_int[p1][i]<<endl;
					cout<<"partner2interface "<<cross_int[p1][i+1]<<endl;
				}
			}
	...
	while (it < maxit) {
		...
		if (it == maxit - 1) {
		  ...
		  for(c=0;c<csize;c++){
		    ...
		    cout <<"p1: "<<p1<<' '<<" nfree; "<<bases[p1].nfree<<' '<<bases[p1].xcom<<' '<<bases[p1].ycom<<' '<<bases[p1].zcom<<'\t';
